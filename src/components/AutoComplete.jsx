...
const AutoComplete = ({allPokemonsData , setDisplayedPokemons }) => {
  const [searchTerm, setSearchTerm] = useState("");


  const filterNames = (input) => { 
    const value = input.toLowerCase(); 
    return value ? allPokemonsData.filter((e) => e.name.includes(value)) 
    : [];
  }
  ...
  return (
    ...
    >
      ...
      <button
      type='submit'
      className='text-xs w-[2.5rem] bg-slate-900 text-slate-300 h-6 px-2 py-1 rounded-r-lg text-center absolute right-0 hover:bg-slate-700'
      >
        검색
      </button>
    </form>
    {checkEqualName(searchTerm).length > 0 && (
      <div className = {
        `w-full flex bottom-0 h-0 flex-col absolute jstify-center items-center translate-y-2` }
        >
        <div className = { 
